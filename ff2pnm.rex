...
/* Classic REXX:  Let ffmpeg decode an image, convert it to PPM   */
...
/* -------------------------------------------------------------- */
   ...
   if TRY = 0        then  exit USAGE( 'unsupported' SRC )
   return MAKEPNM( SRC, DST, OPT )

MAKEPNM: procedure               /* continue, called by TRYIT()   */
   parse arg SRC, PAM, OPT       ;  EOL = x2c( 0A )
   TRY = FFMPIX( SRC, OPT )
   if TRY <> ''      then  exit ERROR( TRY )
   say right( R.0, 9 ) 'bytes,' R.5 'in' R.3 * R.8 * 8 'bits'

   if R.3 = 3 | ( R.3 = 1 & R.4 > 1 )  then  do
      HDR = EOL || R.1 R.2 || EOL || R.4 || EOL
      PAM = left( PAM, length( PAM ) - 3 )
      if R.3 = 1  then  do
         HDR = 'P5' || HDR       ;  PAM = PAM || 'pgm'
      end                        /* PGM for more than two colours */
      else  do                   /* PPM for RGB (no transparency) */
         HDR = 'P6' || HDR       ;  PAM = PAM || 'ppm'
      end
      TRY = stream( PAM, 'c', 'query exists' )
      if TRY <> ''   then  do
         say 'Press [Enter] to overwrite old' TRY
         pull TRY                ;  if TRY <> ''   then  return 1
   ...
   call charout PAM, HDR || R..  ;  call lineout PAM
   say '  created' PAM           ;  return 0
...
USAGE:   procedure               /* show (error +) usage message: */
   ...
   say ' Let ffmpeg convert a given image to PAM, and replace the  '
   say ' P7 header by P5 (PGM) or P6 (PPM) if possible (no alpha). '
