...
set OPTS=-c:v libtheora -dc 10 -b:v 250KiB
set OPTS=%EXEC% -i "%NEED%" %SWSF% %FLTV% %OPTS%
set EXEC=%OPTS% -pass 1 -an -y nul.ogv
echo %EXEC% 1>&2
%EXEC%
...
set EXEC=%OPTS% -pass 2 %FLTA% -c:a libvorbis -b:a 25KiB "%DEST%"
echo %EXEC% 1>&2
%EXEC%
:LOGS --------------------------------------------------------------
...
echo for an OGV conversion (video theora, audio vorbis).
echo/
